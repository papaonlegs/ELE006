function [ boundedPicture ] = QMUL_partC12b( vid, frame)
    %
  ...
  %if we want video
  output = fopen('question12.xml','w');
  fprintf(output,'<Frame> %d', frame);
  ...
  for i=1:length(centre)
      ...
      fprintf(output,'\n\t\t<Displacement> %.00f %.00f %.00f </Displacement>', xdis(index),ydis(index), minDis);
